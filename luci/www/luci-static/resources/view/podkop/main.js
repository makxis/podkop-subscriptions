...
var PODKOP_LUCI_APP_VERSION = "v0.7.17";
...
function podkopSubIsRealDashboardSection(section) {
  const code = String(
    section && (section.code || section.displayName || section.name || section.tag) || ""
  );

  return !/^cfg[0-9a-f]{6}$/i.test(code);
}
...
async function renderSectionsWidget() {
  ...
  const renderedWidgets = (sectionsWidget.data || []).filter(podkopSubIsRealDashboardSection).map(
...
async function runSectionsCheck() {
  ...
  const items = await Promise.all(
    (sections.data || []).filter(podkopSubIsRealDashboardSection).map(async (section) => {
