...
return baseclass.extend({
  createSubscriptionsContent: function(section, podkopSections, version) {
    ...
    o = ss.option(
      ...
      _("Если домен узла указывает на несколько адресов, к списку добавляются копии ключа с каждым IP, а исходный ключ с доменом остаётся. Нужно затем, чтобы URLTest выбирал конкретный самый быстрый сервер, а не тот, который в этот раз вернул DNS. Домены с единственным адресом не трогаются. К имени дописывается последний октет, чтобы записи различались. Учтите: ключей станет заметно больше, и лимит «Максимум ключей в секции» будет выбираться быстрее.")
    );
    o.default = "0";
    o.rmempty = false;
