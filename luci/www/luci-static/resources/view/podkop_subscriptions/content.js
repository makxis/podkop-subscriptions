...
const PODKOP_SUBSCRIPTIONS_VERSION_FALLBACK = "3.7.4";
...
return baseclass.extend({
  createSubscriptionsContent: function(section, podkopSections, version) {
    ...
    o = ss.option(
      form.Flag,
      "expand_domain_ips",
      _("Разворачивать домены в IP"),
      _("Домен с несколькими адресами даёт по ключу на каждый IP, доменный ключ остаётся. URLTest тогда выбирает самый быстрый сервер, а не случайный. Ключей станет заметно больше — следите за лимитом.")
    );
    o.default = "0";
    o.rmempty = false;
    ...
    o = section.option(
      ...
      "fingerprint",
      _("Отпечаток клиента"),
      _("Заголовки, по которым панель узнаёт настоящее приложение. Профиль default применяется ко всем группам. Если панель не признала один профиль, updater пробует следующий.")
    );
    ...
    o = ss.option(
      ...
      "enabled",
      _("Включено"),
      _("Отключённый профиль не используется.")
    );
    o.default = "1";
    o.rmempty = false;
    ...
    o = ss.option(
      ...
      "_headers",
      _("Заголовки"),
      _("По заголовку в строке, «Имя: значение». Порядок и регистр важны — панель смотрит на них. Вместо {hwid} при первом обновлении подставится идентификатор этого роутера. Host и Accept-Encoding можно не убирать.") +
        "<br><br>" +
        _("Встроенный набор:") +
