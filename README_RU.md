...
| JSON с объектами Clash/Mihomo | Sub-Store с `?target=JSON` |
| JSON с полными конфигами Xray | Remnawave: массив конфигов, имя узла в `remarks` |

Разбор JSON не требует `python3-yaml`: объекты Clash приходят именно как JSON.
Правила сборки ссылки перенесены из Sub-Store (`producers/uri.js`) — того
самого кода, которым панель формирует `?target=URI`, — и сверены с ним: одна и
та же коллекция в обоих форматах даёт одинаковые ссылки.
