...
Отдельный необязательный скрипт (`install-dnsproxy.sh`, версия **1.3.0**), к подпискам прямого отношения не имеющий. У него своя нумерация, не связанная с версией Podkop Subscriptions. Ставит и настраивает AdGuard dnsproxy на `127.0.0.10:53`, добавляет защищённые upstream-серверы, а DNS Podkop направляет на себя.

Установленную версию печатает сам скрипт в конце работы, строкой `Версия установщика: 1.3.0`.
