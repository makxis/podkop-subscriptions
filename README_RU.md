...
- ходит за подписками с отпечатком настоящего мобильного клиента, задаваемым в LuCI;
- пробует несколько профилей отпечатка, пока панель не отдаст список;
- распознаёт заглушки и узлы-пустышки, которыми панель сообщает об отказе;
- умеет разворачивать домен узла в отдельные ключи по его IP-адресам;
...
| `expand_domain_ips` | `1` | Домен, указывающий на несколько адресов, разворачивается в отдельный ключ на каждый IP. Доменный ключ остаётся. `0` — выключить. См. [Разворачивание доменов в IP](#разворачивание-доменов-в-ip). |
| `fingerprint` | пусто | Имя профиля отпечатка для этой группы. Пусто — профиль `default`. См. [Отпечаток клиента](#отпечаток-клиента). |
| `fingerprint_probe_days` | `7` | Как часто перезамерять, какой профиль даёт больше узлов. `0` — замерить один раз и больше не трогать. |
...
Панели выдачи подписок (Remnawave, Marzban, 3x-ui и подобные) отдают разное
содержимое разным клиентам, опознавая их по заголовкам запроса: `User-Agent`,
`X-HWID`, `X-Device-Model`, `X-App-Version`. Обычному HTTP-клиенту такая панель
вернёт усечённый список, заглушку или откажет вовсе.

Поэтому updater ходит за подписками набором заголовков, снятым с настоящего
мобильного клиента. Набор задаётся в конфиге и правится в LuCI:
...
Порядок строк и регистр имён сохраняются как есть: панель смотрит и на то, и
на другое, поэтому набор вставляется ровно в том виде, в каком снят. `Host` и
`Accept-Encoding` можно не убирать, updater отбросит их сам.

Профиль `default` применяется ко всем группам подписок. Группа может выбрать
другой опцией `fingerprint`.

### X-HWID

`X-HWID` — обычная строка блока. Значение `{hwid}` означает «сгенерировать
персональный идентификатор при первом обновлении и записать сюда»: 16
шестнадцатеричных символов из `/dev/urandom`. Дальше значение не меняется —
для панели смена `X-HWID` выглядит как появление нового устройства.

Если секции `fingerprint` в конфиге нет вовсе, она создаётся при первом
запуске со встроенным набором и собственным HWID.

**Лимит устройств.** Многие панели ограничивают число устройств на подписку и
привязывают их именно по `X-HWID`. Каждый роутер со своим идентификатором
занимает отдельный слот. Если лимит мешает, скопируйте уже сгенерированное
значение с первого роутера на остальные — тогда они будут выглядеть одним
устройством.

### Несколько профилей

Профилей может быть несколько: одна панель ждёт v2raytun, другая Happ. Задавать
соответствие руками не нужно — updater пробует их по очереди, пока подписка не
прочитается. Первым идёт профиль, выбранный у группы, затем остальные в порядке
их следования в конфиге.

### Какой профиль выбирается

Профили не просто перебираются до первого удачного — при первом чтении
источника updater опрашивает каждый и запоминает тот, что дал больше узлов.
Разница бывает существенной: на одной и той же подписке один клиент получает
27 узлов, другой 30.

Замер стоит по запросу на профиль, поэтому делается редко. Обычный прогон
обходится одним запросом и берёт запомненный профиль. Перезамер происходит,
когда выбора ещё не было, когда профиль исчез из конфига, когда прошёл срок из
`fingerprint_probe_days`, или когда запомненный профиль вдруг дал заметно
меньше узлов — последнее означает, что панель изменила поведение.

**Отказы запоминаются отдельно.** Если панель отказала конкретному профилю, он
исключается из будущих замеров на этом источнике. Это не только экономия
запросов: на панели с лимитом устройств сама попытка чужим клиентом занимает
слот, потому что различают клиентов не только по `X-HWID`, но и по
`User-Agent`.

Неудачей считается не только ошибка загрузки. Панель, не признавшая клиента,
обычно отвечает кодом 200: либо страницей-заглушкой антибота, либо фиктивным
узлом на `0.0.0.0:1`, у которого причина написана в имени, например
«Вы достигли максимального числа устройств для вашей подписки». Такие ответы
распознаются, причина попадает в лог, а ключи-пустышки в конфиг Podkop не
записываются.

### Чем отправляется запрос

При наличии `curl` запрос идёт им: он отправляет заголовки байт в байт и в
заданном порядке. Отдельной зависимости это не создаёт, `curl` требуется
самому Podkop.

Штатный для OpenWrt `wget` (это `uclient-fetch`) остаётся запасным путём, но он
всегда переписывает `User-Agent` своим написанием и ставит его последним, так
что отпечаток получается приблизительным. В логе на этот случай есть
предупреждение.

Сжатие не запрашивается: libcurl в OpenWrt собран без zlib.

---

## Форматы подписок

Распознаются автоматически, в таком порядке: base64 → прямые ссылки → JSON.

| Формат | Пример источника |
|---|---|
| прямые ссылки по строкам | большинство панелей |
| base64 от такого списка | он же, обычная упаковка |
| JSON с объектами Clash/Mihomo | панели и конвертеры, отдающие список узлов в JSON |
| JSON с полными конфигами Xray | массив конфигов, по одному на узел, имя в `remarks` |

Разбор JSON не требует `python3-yaml`: объекты Clash приходят именно как JSON.
Правила сборки ссылки не выдуманы, а перенесены из зрелой открытой реализации
и сверены с её выводом на одних и тех же данных.

Если источник умеет отдавать готовые ссылки, лучше просить у него именно их:
меньше преобразований — меньше поводов разойтись.

---

## Номер источника в имени ключа

К имени каждого ключа, пришедшего из подписки, дописывается номер источника в
том же порядке, в каком они перечислены в группе: `[2] 🇳🇱 Нидерланды`. В
списке Podkop сразу видно, откуда приехал узел, а по номеру он соотносится со
строками лога вида «источник 2».

Номер обновляется на каждом запуске, а не накапливается: если порядок
источников в группе поменялся, ключи получат новые номера. Ключи, уже лежащие
в секции, тоже переименовываются, когда тот же узел снова приходит из подписки.

Имя не участвует в идентификации ключа (`stable_id` считается по ссылке без
части после `#`), поэтому переименование не создаёт новых ключей и не
сбрасывает накопленную статистику отказов.

Локальные ключи из `local-links` номера не получают: они не из подписки.

---

## Разворачивание доменов в IP

Подписка часто указывает узел доменом, а за ним стоит несколько серверов. Какой
из них достанется, решает DNS, и это не обязательно самый быстрый.

Опция `expand_domain_ips` (галочка **Разворачивать домены в IP** в LuCI) для
домена с двумя и более адресами добавляет по ключу на каждый IP. Доменный ключ
остаётся: он продолжит работать, когда адреса сменятся. URLTest в Podkop затем
измеряет серверы по отдельности и выбирает лучший.

Подменяется только хост. `sni`, `host` и остальные параметры остаются от
домена, иначе TLS и ws-транспорт сломаются. К имени дописывается последний
октет адреса, а если он у адресов одинаковый — адрес целиком.

Домены с единственным адресом не трогаются. Разворачивание идёт после
regex-фильтра, поэтому отсеянные ссылки не стоят DNS-запросов.

Ключей становится заметно больше, так что лимит `max_links` выбирается быстрее.

Опция включена по умолчанию, в том числе для конфигов, где её нет вовсе: после
обновления такая группа начнёт разворачивать домены. Выключается явным
`option expand_domain_ips '0'` в группе или снятой галкой в LuCI — выключение
сохраняется и при следующих обновлениях.
...
Похоже на дублирование, но проверки отвечают на разные вопросы, и первая не слабее второй.

Ссылку в outbound превращает **сам Podkop**, конвертером
`/usr/lib/podkop/sing_box_config_facade.sh`. Он и есть потолок возможностей, а
не sing-box: даже то, что sing-box умеет, бесполезно, если Podkop не умеет это
собрать. Поэтому первая проверка повторяет ровно его `case`-ы — по схеме, по
`security` и по `type`.

Что происходит без неё, проверено на роутере:
...
То есть в первом случае ключ молча превращается в нерабочий узел, который
честно висит в URLTest, а конфиг при этом валиден и вторую проверку проходит. Во
втором один такой ключ оставляет роутер вообще без proxy. Поймать оба случая
можно только до конвертации.

Вторая проверка ловит то, что до конвертации не видно: неизвестный метод
шифрования у shadowsocks, reality без uTLS и прочее, что вылезает уже при
разборе готового конфига.

Заодно первая проверка и есть то, что **строит** конфиг для второй: `sing-box
check` читает JSON, а не ссылки, и без конвертера проверять было бы нечего.

Какой ключ и на каком значении отвалился, видно в логе на уровне DEBUG:
