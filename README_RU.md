...
Основная команда, ради которой всё и нужно. Скачивает подписки, фильтрует, проверяет и записывает ключи в Podkop:

```sh
/usr/bin/podkop-sub-run-now
```

Запускается в фоне и сразу возвращает управление. Посмотреть, чем закончилось:
...
Если нужен синхронный запуск (скрипт ждёт завершения и возвращает код выхода) — вызывайте updater напрямую:
...
В LuCI то же самое делает кнопка **Запустить updater** внизу страницы. До первого запуска страница показывает:
...
| Команда | Что делает |
|---|---|
| `/usr/bin/podkop-sub-run-now` | **Обновить ключи сейчас.** Запускает полный цикл (загрузка → фильтр → валидация → запись в Podkop) в фоне, пишет в `/tmp/podkop-sub-updater.log`. Если updater уже работает, запуск тихо пропускается. |
| `/usr/bin/podkop-sub-run-now --status` | Состояние ручного запуска: `running` / `finished` / `idle` плюс хвост лога. |
...
| `/usr/bin/podkop-sub-updater.py --subs /etc/config/podkop_subscriptions --config /etc/config/podkop --force` | Синхронный полный прогон с принудительной перезаписью секции и перезапуском Podkop. Основной способ отладки: весь вывод идёт в терминал. |
