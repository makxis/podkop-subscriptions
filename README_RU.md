...
Это важно: расписание в `/etc/config/podkop_subscriptions` — это только сохранённая настройка. Реально выполняется только то, что есть в `/etc/crontabs/root`. Если в настройках расписание есть, а в cron его нет, страница покажет `автообновление: не применено`.
