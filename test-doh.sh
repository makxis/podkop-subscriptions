#!/bin/sh
# Проверка DoH/DoT/DoQ upstream-серверов без Python: только dnsproxy и
# nslookup, которые и так нужны install-dnsproxy.sh. Сделан для случая, когда
# на роутере не установлен python3-компонент подписок (или установлен не он,
# а только сам dnsproxy), а сравнить upstream всё равно хочется.
#
# Тестирует список последовательно, по одному серверу за раз — без пула
# воркеров, как в test-doh.py. Полный параллелизм на busybox ash (wait -n,
# слежение за несколькими pid) значительно сложнее и хрупче на разных
# версиях busybox; для разового прогона тест на 30-40 серверов и без этого
# укладывается в разумное время.
set -eu
